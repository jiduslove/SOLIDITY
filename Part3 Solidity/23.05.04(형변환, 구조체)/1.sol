...
contract UINT {
// 1바이트는 16진수를 2자리까지 쓸 수 있다.
// 16진수 100이 10진수 256 (16진수 10이 10진수 16, 20=32, 30=48, )
// 용량을 구분하는 이유는 가스비 절감을 위해서.
// uint로 용량제한없이 작성하면 기본적으로 uint256으로 인식한다.
    uint a; // 1바이트 기준으로 움직임.
    uint8 b; // 8bits = 1 bytes = 16진수 2자리 = 0 ~ ff(16진수) = 0 ~ 255(10진수)
    uint16 c;
    uint256 d; 
